...
// Returns true if std::get on ButtonType is ::none, otherwise false.
// Returns false if enum value in the ButtonType is not ::none, or if there's no way to obtain
// an enum value, for example because passed type is std::monostate.
template<typename TButtonType>
inline bool isNone(ButtonType type) {
        ...
        if constexpr (std::is_same_v<TButtonType, std::monostate>) {
                ...
                // If ButtonType is already set to std::monostate then there's no enum to enum
                // to compare to.
